...
echo "[<SJ_STORAGE_HOST>:ROOT]" > ~/.nsmbrc
echo "password=<TRUENAS_ROOT_PASSWORD>" >> ~/.nsmbrc
cat ~/.nsmbrc
mkdir /mnt/sj-storage
mount_smbfs -N -I <SJ_STORAGE_IP> //root@<SJ_STORAGE_IP>/sj-storage/ /mnt/sj-storage/ || mount -t cifs -o vers=3,username=root,password=<TRUENAS_ROOT_PASSWORD> '//<SJ_STORAGE_IP>/sj-storage/' /mnt/sj-storage/
