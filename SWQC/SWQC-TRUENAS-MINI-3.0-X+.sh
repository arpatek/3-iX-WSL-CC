...
# Grabbing Motherboard Info (Redbook Pg. 18)
...
# TrueNAS Mini X+ BIOS Firmware Check

if [[ $BIOVER == *"1.3.V1"* ]]; then
    echo "  - BIOS Version For $PRODUCT Is Correctly Showing As $BIOVER" >>swqc-tmp/"$SERIAL"-BIOS-Version-Check.txt
else
    echo "  - BIOS Version For $PRODUCT Is Showing As $BIOVER It Should Be 1.3.V1" >>swqc-tmp/"$SERIAL"-BIOS-Version-Check.txt
fi
...
# TrueNAS Mini X+ BMC Firmware Check

if [[ $BMCINFO == *"3.60"* ]]; then
    echo "  - BMC Version For $PRODUCT Is Correctly Showing As $BMCINFO" >>swqc-tmp/"$SERIAL"-BMC-Version-Check.txt
else
    echo "  - BMC Version For $PRODUCT Is Showing As $BMCINFO It Should Be 3.60" >>swqc-tmp/"$SERIAL"-BMC-Version-Check.txt
fi
...
if pciconf -lvcb | grep "T520"; then
    pciconf -lvcb | grep "T520" | sort -u | cut -d "'" -f 2 >swqc-tmp/"$SERIAL"-10G-Card.txt
else
...
# Getting Boot Device Info (Redbook Pg. 19)
...
# Checdking Fan Threshold Is Correctly Set to 200 300 500 (Redbook Pg. 48)

ipmitool sensor list | grep -i 'FAN[A]' | cut -d "|" -f1 -f5 -f6 -f7 | xargs >swqc-tmp/"$SERIAL"-FAN-Thresh.txt
FANTHRESH=$(cat swqc-tmp/"$SERIAL"-FAN-Thresh.txt)

FANSETTINGS="FANA | 200.000 | 300.000 | 500.000"

if [[ $FANTHRESH == "$FANSETTINGS" ]]; then
    echo "Fan Theshold Correctly Set To 200 300 500" >swqc-tmp/"$SERIAL"-FAN-Check.txt
else
    echo "Fan Theshold Not Set To 200 300 500" >swqc-tmp/"$SERIAL"-FAN-Check.txt
fi
FANCHECK=$(cat swqc-tmp/"$SERIAL"-FAN-Check.txt)

# Checking Fan Speed Is Correctly Set to 00 (Redbook Pg. 48)
...
if [[ $FANSPEED == 00 ]]; then
    echo "Fan Speed Correctly Set To Standard ($FANSPEED)" >swqc-tmp/"$SERIAL"-FAN-Check.txt
else
    echo "Fan Speed Not Set To Standard (00) It Is Showing As $FANSET ($FANSPEED)" >>swqc-tmp/"$SERIAL"-FAN-Check.txt
    echo "Use Command: ipmitool raw 0x30 0x45 0x01 0x00 To Set To Standard" >>swqc-tmp/"$SERIAL"-FAN-Check.txt
fi
