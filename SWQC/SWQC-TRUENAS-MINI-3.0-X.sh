...
if [[ $BIOVER == *"L1.42A"* ]]; then
    echo "  - BIOS Version For $PRODUCT Is Correctly Showing As $BIOVER" >>swqc-tmp/"$SERIAL"-BIOS-Version-Check.txt
else
    echo "  - BIOS Version For $PRODUCT Is Showing As $BIOVER It Should Be L1.42A" >>swqc-tmp/"$SERIAL"-BIOS-Version-Check.txt
fi
...
if [[ $BMCINFO == *"1.60"* ]]; then
    echo "  - BMC Version For $PRODUCT Is Correctly Showing As $BMCINFO" >>swqc-tmp/"$SERIAL"-BMC-Version-Check.txt
else
    echo "  - BMC Version For $PRODUCT Is Showing As $BMCINFO It Should Be 1.60" >>swqc-tmp/"$SERIAL"-BMC-Version-Check.txt
fi
