...
# Grabbing BIOS Version (Redbook Pg. 4)
...
# Grabbing BMC Firmware Version (Redbook Pg. 4)
...
# TrueNAS R20B BMC Firmware Check
...
# Checking Fan Speed Is Correctly Set to 04 (Redbook Pg. 29)
...
if [[ $FANSPEED == 04 ]]; then
    echo "Fan Speed Correctly Set To HeavyIO ($FANSPEED)" >swqc-tmp/"$SERIAL"-FAN-Check.txt
else
    echo "Fan Speed Not Set To HeavyIO (04) It Is Showing As $FANSET ($FANSPEED)" >>swqc-tmp/"$SERIAL"-FAN-Check.txt
    echo "Use Command: ipmitool raw 0x30 0x45 0x01 0x04 To Set To HeavyIO" >>swqc-tmp/"$SERIAL"-FAN-Check.txt
fi
FANSPEEDCHECK=$(cat swqc-tmp/"$SERIAL"-FAN-Check.txt)

# Checking That FAN A,B,2,3,4 Are Installed (Redbook Pg. 23)

ipmitool sensor list | grep -i 'FAN[AB234]' | cut -d "|" -f 4 | grep -c ok >swqc-tmp/"$SERIAL"-FAN-Count.txt
FANCOUNT=$(cat swqc-tmp/"$SERIAL"-FAN-Count.txt)

if [[ $FANCOUNT -eq 5 ]]; then
    echo "Fans AB234 Are Connected" >swqc-tmp/"$SERIAL"-FAN-Count-Check.txt
else
    echo "Check Fans AB234" >swqc-tmp/"$SERIAL"-FAN-Count-Check.txt
fi
FANCOUNTCHECK=$(cat swqc-tmp/"$SERIAL"-FAN-Count-Check.txt)

# Checking Fan Threshold Is Correctly Set to 100 200 200

ipmitool sensor list | grep -i 'FAN[234]' | cut -d "|" -f1 -f5 -f6 -f7 | xargs >swqc-tmp/"$SERIAL"-FAN-Thresh.txt
FANTHRESH=$(cat swqc-tmp/"$SERIAL"-FAN-Thresh.txt)

FANSETTINGS="FAN2 | 100.000 | 200.000 | 200.000 FAN3 | 100.000 | 200.000 | 200.000 FAN4 | 100.000 | 200.000 | 200.000"

if [[ $FANTHRESH == "$FANSETTINGS" ]]; then
    echo "Fan Theshold Correctly Set To 100 200 200" >swqc-tmp/"$SERIAL"-FAN-Check.txt
else
    echo "Fan Theshold Not Set To 100 200 200" >swqc-tmp/"$SERIAL"-FAN-Check.txt
fi
