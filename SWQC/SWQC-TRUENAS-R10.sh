...
# TrueNAS R10 BIOS Firmware Check

if echo "$BIOVER" | grep -oh "\w*3.3\w*" | grep -Fwqi -e 3.3; then
    echo "  - BIOS Version For $PRODUCT Is Correctly Showing As $BIOVER" >>swqc-tmp/"$SERIAL"-BIOS-Version-Check.txt
else
    echo "  - BIOS Version For $PRODUCT Is Showing As $BIOVER It Should Be 3.3" >>swqc-tmp/"$SERIAL"-BIOS-Version-Check.txt
fi
...
# TrueNAS R10 BMC Firmware Check

if echo "$BMCINFO" | grep -oh "\w*1.71\w*" | grep -Fwqi -e 1.71; then
    echo "  - BMC Version For $PRODUCT Is Correctly Showing As $BMCINFO" >>swqc-tmp/"$SERIAL"-BMC-Version-Check.txt
else
    echo "  - BMC Version For $PRODUCT Is Showing As $BMCINFO It Should Be 1.71" >>swqc-tmp/"$SERIAL"-BMC-Version-Check.txt
fi
...
if [[ $FANSPEED == 00 ]]; then
    ...
    echo "Use Command: ipmitool raw 0x30 0x45 0x01 0x04 To Set To Standard" >>swqc-tmp/"$SERIAL"-FAN-Check.txt
fi
FANSPEEDCHECK=$(cat swqc-tmp/"$SERIAL"-FAN-Check.txt)

# Checking That FAN A,B,2,3,4 Are Installed (Redbook Pg. 23)

ipmitool sensor list | grep -i 'FAN[AB234]' | cut -d "|" -f 4 | grep -i ok | wc -l | xargs >swqc-tmp/"$SERIAL"-FAN-Count.txt
